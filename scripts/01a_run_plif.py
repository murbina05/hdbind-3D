"""
Compute PLIF fingerprints for all 43 DUDE-Z targets in parallel.

Usage:
    python scripts/01a_run_plif.py [--targets TARGET [TARGET ...]] [--n-workers N]
"""
...
def main(targets: list[str], n_workers: int) -> None:
    ...
    log_gpu_status()
    log.info("Running PLIF on %d targets with %d workers", len(targets), n_workers)

    featurizer = PLIFFeaturizer()
    with timer("PLIF total"):
        with ProcessPoolExecutor(max_workers=n_workers) as pool:
            futures = {pool.submit(process_target, t): t for t in targets}
            for future in tqdm(as_completed(futures), total=len(futures),
                               desc="PLIF", unit="target"):
