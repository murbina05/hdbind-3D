"""
Compute distance+chemistry hybrid features for all 43 DUDE-Z targets.
Combines GPU distance matrices with residue physicochemical properties.
...
Usage:
    python scripts/02c_run_dist_chem.py [--targets TARGET [TARGET ...]] [--n-workers N]
"""
...
def process_target(target: str) -> tuple[str, dict | None, float, float]:
    ...
    try:
        ...
        featurizer = DistanceChemFeaturizer()
...
def main(targets: list[str], n_workers: int) -> None:
    ...
    log_gpu_status()
    log.info("Running DistanceChemFeaturizer on %d targets", len(targets))
    ...
    with timer("Distance total"):
        for target in tqdm(targets, desc="DistChem", unit="target"):
            if _cuda:
