"""
Plot 1v — Featurizer accuracy overview (violin variant).

Identical layout to plot_01_featurizer_accuracy_overview.py but uses
sns.violinplot instead of sns.boxplot in each cell, with per-target dots
overlaid via sns.stripplot.

4-row × 3-column grid of horizontal violin plots + strip plots.
...
Outputs (both vector and raster):
  results/figures/summary/plot_01v_featurizer_accuracy_violin.pdf
  results/figures/summary/plot_01v_featurizer_accuracy_violin.png

Usage:
    python scripts/plot_01v_featurizer_accuracy_violin.py \\
...
_LOG_FILE = RESULTS_DIR / "logs" / "plot_01v_featurizer_accuracy_violin.log"
...
def _draw_cell(
    ...
    """Draw one violinplot + strip cell."""
    ...
    with warnings.catch_warnings():
        ...
        sns.violinplot(
            ...
            ax=ax,
            width=0.8,
            inner="box",   # miniature IQR box + median line inside each violin
            cut=0,         # clip density to data range — no tails past observed min/max
            linewidth=0.8,
            alpha=0.75,
        )
        sns.stripplot(
...
def make_plot(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    _save(fig, output_dir, "plot_01v_featurizer_accuracy_violin")

    n_feats = df["featurizer"].nunique()
    n_tgt   = df["target"].nunique()
    print(
        f"\nPlot 1v (violin): {n_feats} featurizers × {len(DECOY_SETS)} decoy sets × "
...
def main(results_dir: Path, comparison_csv: Path, output_dir: Path) -> None:
    ...
    with timer("Plot 1v — featurizer accuracy violin"):
